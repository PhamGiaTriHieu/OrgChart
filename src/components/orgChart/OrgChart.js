...
export class OrgChart {
  constructor() {
    ...
    const attrs = {
      ...
      buttonContent: ({node, state}) => {
        ...
        return `<div style="border:1px solid #3a94ee;border-radius:3px;padding:3px;font-size:9px;margin:auto auto;background-color:white"> ${icons[
          state.layout
        ](node.children)}  </div>`;
      },
      ...
      linkUpdate: function (d, i, arr) {
        d3.select(this)
          .attr('stroke', (d) =>
            d.data._upToTheRootHighlighted ? '#E27396' : '#3a94ee'
          )
          .attr('stroke-width', (d) =>
  ...
  addNode(obj) {
    ...
    const nodeFound = descendants.filter(
      ({data}) => attrs.nodeId(data).toString() === attrs.nodeId(obj).toString()
    )[0];
      ...
      if (node.children) {
        node.children.forEach((child, j) => {
          ...
          if (
            ...
            while (
              localNode &&
              (hiddenNodesMap[localNode.id] || localNode.data._pagingButton)
            ) {
